...
// Source File Name:   InsertJspBuilder.java
	...
	implements CodeBuilder
{
	...
	public InsertJspBuilder(Project project)
	{
	...
	private void saveToFile(StringBuffer codeBuffer)
	{
		...
		String fileName = (new StringBuilder(String.valueOf(project.getOutputPath()))).append("view").append(File.separator).append(entity.getEntityBeanName()).append(File.separator).append("insert").append(entity.getEntityName()).append(".html").toString();
	...
	public void saveToFile()
	{
		...
		String file = (new StringBuilder(String.valueOf(project.getTemplatePath()))).append("insertHeader.txt").toString();
		String actionString;
		try
		{
		...
		actionString = actionString.replaceAll("_InsertFormParam_", searchAreaParam.toString());
		actionBuffer.append(actionString);
	...
	private String createOneProperty(StringBuffer searchAreaParam,Field field){
		...
		if(field.getNullAble()==0){//不可为空
			...
			if(getDataType(field.getDataType()).equals("Date")){//日期类型
				searchAreaParam.append("                            ").append((new StringBuilder("<td><input id='")).append(field.getPropertyName()).append("'  name='"+field.getPropertyName()+"' class=\"required date\" readonly='true' onclick=\"laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})\"/></td>\r\n").toString());
			}
			//number类型
			...
			//字符串类型
				 ...
				 if(field.getLength()>=100){//超过100生成textarea
					 searchAreaParam.append("                            ").append((new StringBuilder("<td><textarea id='")).append(field.getPropertyName()).append("' name='"+field.getPropertyName()+"' class=\"{required:true,maxlength:"+field.getLength()+"}\"  rows=\"2\" ></textarea></td>\r\n").toString());
				 }else{
					 searchAreaParam.append("                            ").append((new StringBuilder("<td><input id='")).append(field.getPropertyName()).append("' name='"+field.getPropertyName()+"' class=\"{required:true,maxlength:"+field.getLength()+"}\"/></td>\r\n").toString());
				 }
			}
		}else{//可为空
			//日期类型
			if(getDataType(field.getDataType()).equals("Date")){//日期类型
				searchAreaParam.append("                            ").append((new StringBuilder("<td><input id='")).append(field.getPropertyName()).append("'  name='"+field.getPropertyName()+"' readonly='true' onclick=\"laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})\"/></td>\r\n").toString());
			}
			//number类型
			else if(getDataType(field.getDataType()).equals("Integer")||getDataType(field.getDataType()).equals("double")||getDataType(field.getDataType()).equals("BigDecimal")){
				searchAreaParam.append("                            ").append((new StringBuilder("<td><input id='")).append(field.getPropertyName()).append("'  name='"+field.getPropertyName()+"' /></td>\r\n").toString());
			}
			//字符串类型
			else{
				 if(field.getLength()>=100){//超过100生成textarea
					 searchAreaParam.append("                            ").append((new StringBuilder("<td><textarea id='")).append(field.getPropertyName()).append("'  name='"+field.getPropertyName()+"'  rows=\"2\" ></textarea></td>\r\n").toString());
				 }else{
					 searchAreaParam.append("                            ").append((new StringBuilder("<td><input id='")).append(field.getPropertyName()).append("' name='"+field.getPropertyName()+"' /></td>\r\n").toString());
				 }
			}
